...
void PlayState::calculateShotTime(double fts)
{
	if (m_shotTime < 0.0)
	{
		m_gunDisplay.setTexture(&m_textureGun);
...
void PlayState::handleShot()
{
	...
	{
		if (m_clickables[i].getDestructible() && m_clickables[i].containsVector(m_crosshair.getPosition()))
		{
...
void PlayState::handleMouselook(const sf::Vector2f mouseMovePos)
{
